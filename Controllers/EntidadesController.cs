...
namespace ApiGenericaCsharp.Controllers
{
    ...
    public class EntidadesController : ControllerBase
    {
        ...
        public async Task<IActionResult> CrearAsync(
            string tabla,                                           // Del path: /api/{tabla}
            [FromForm] IFormCollection formData,                    // Del form: datos del formulario
        ...
        )
        {
            try
            {
                ...
                _logger.LogInformation("Datos recibidos en formulario: {DatosCount} campos", formData.Count);
                
                // CONVERSIÓN DE IFormCollection A DICTIONARY
                var datosEntidad = new Dictionary<string, object?>();
                foreach (var kvp in formData)
                {
                    var valor = formData[kvp.Key].ToString();
                    datosEntidad[kvp.Key] = valor;
                    _logger.LogInformation("Campo: {Campo}, Valor: {Valor}, Tipo: {Tipo}", 
                        kvp.Key, valor, valor?.GetType().Name ?? "null");
                }
                ...
                // VALIDACIÓN DE CAMPOS OBLIGATORIOS (NO NULOS NI VACÍOS)
                foreach (var kvp in datosConvertidos)
                {
                    if (kvp.Value == null || (kvp.Value is string str && string.IsNullOrWhiteSpace(str)))
                    {
                        ...
                        {
                            estado = 400,
                            mensaje = $"El campo '{kvp.Key}' es obligatorio y no puede estar vacío.",
                            tabla = tabla,
                            campo = kvp.Key
                        });
                    }
